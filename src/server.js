    ...
    io.on("connection", (socket) => {
      ...
      socket.on("join", async (userId) => {
        ...
        try {
          ...
          console.log(
            `Initial friend requests for user ${userId}:`,
            friendRequests
          );
      ...
      socket.on(
        "send_friend_request",
        async ({ senderId, receiverId, username }) => {
          try {
            console.log(
              `Received friend request from ${senderId} to ${receiverId} (${username})`
            );
      ...
      socket.on("accept_friend_request", async ({ userId, friendId }) => {
        try {
          ...
          user.waiting_list = user.waiting_list.filter(
            (id) => id.toString() !== friendId
          );
          friend.invite_list = friend.invite_list.filter(
          ...
          console.log(
            `Friend request accepted between ${userId} and ${friendId}`
          );
      ...
      socket.on("reject_friend_request", async ({ userId, friendId }) => {
        try {
          ...
          console.log(
            `Friend request rejected between ${userId} and ${friendId}`
          );
      ...
      socket.on("unfriend", async ({ userId, friendId }) => {
        try {
          ...
          io.to(friendId).emit("unfriend", { userId, friendId });
          io.to(userId).emit("unfriend", { userId, friendId });
